...
int wsio_setoption(CONCRETE_IO_HANDLE ws_io, const char* optionName, const void* value)
{
    ...
    else
    {
        ...
        /* Codes_SRS_WSIO_01_156: [ All options shall be passed as they are to uws by calling `uws_client_set_option`. ]*/
...
OPTIONHANDLER_HANDLE wsio_retrieveoptions(CONCRETE_IO_HANDLE handle)
{
    ...
    else
    {
        ...
        /* Codes_SRS_WSIO_01_119: [ `wsio_retrieveoptions` shall call `uws_client_retrieve_options` to produce an `OPTIONHANDLER_HANDLE` and on success return the new `OPTIONHANDLER_HANDLE` handle. ]*/
        result = uws_client_retrieve_options(wsio->uws);
        if (result == NULL)
        {
            /* Codes_SRS_WSIO_01_120: [ If `uws_retrieveoptions` fails then `wsio_retrieveoptions` shall fail and return NULL.  ]*/
            LogError("uws_client_retrieve_options failed");
        }
    }
